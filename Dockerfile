...
WORKDIR /src
COPY ["OOP-3.csproj", "."]
RUN dotnet restore "./OOP-3.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./OOP-3.csproj" -c %BUILD_CONFIGURATION% -o /app/build

# Questa fase viene usata per pubblicare il progetto di servizio da copiare nella fase finale
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./OOP-3.csproj" -c %BUILD_CONFIGURATION% -o /app/publish /p:UseAppHost=false
